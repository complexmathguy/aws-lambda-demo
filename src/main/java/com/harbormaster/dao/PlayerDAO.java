...
// ~AIB
{
    ...
    throws ProcessingException
    {
	    ...
	    StringBuilder fromClause = new StringBuilder( "from com.harbormaster.bo.Player as player where " );
...
// AIB : #getHibernateFindFromClause()
			fromClause.append( "player.playerId = " + pk.getPlayerId().toString() );
// ~AIB
		...
		finally
		{
			...
			{		
				LOGGER.info( "PlayerDAO.findPlayer - Hibernate failed to close the Session - " + exc );
			}
		}		    
	    ...
	    throws ProcessingException
	    {
			...
			finally
			{
				...
				{		
					LOGGER.info( "PlayerDAO.findAllPlayer - Hibernate failed to close the Session - " + exc );
				}		
			}
	    ...
	    throws ProcessingException
	    {
			...
			catch( Throwable exc )
			{
				...
				{
					LOGGER.info( "PlayerDAO.createPlayer - Hibernate failed to rollback - " + exc1 );
				}
				exc.printStackTrace();			
				throw new ProcessingException( "PlayerDAO.createPlayer failed - " + exc );
			}		
			finally
			{
				try
				...
				{		
					LOGGER.info( "PlayerDAO.createPlayer - Hibernate failed to close the Session - " + exc );
				}
			}		
    ...
    throws ProcessingException
    {
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "PlayerDAO.savePlayer - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
			throw new ProcessingException( "PlayerDAO.savePlayer failed - " + exc );		
		}		
		finally
		{
			try
			...
			{		
				LOGGER.info( "PlayerDAO.savePlayer - Hibernate failed to close the Session - " + exc );
			}
		}		    
        return( businessObject );
    }
    ...
    * @exception    ProcessingException
    */
    public void deletePlayer( PlayerPrimaryKey pk ) 
    throws ProcessingException 
    {
	    Transaction tx 	= null;
	    Session session = null;
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "PlayerDAO.deletePlayer - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
		...
		finally
		{
			...
			{		
				LOGGER.info( "PlayerDAO.deletePlayer - Hibernate failed to close the Session - " + exc );
			}
		}
    }
