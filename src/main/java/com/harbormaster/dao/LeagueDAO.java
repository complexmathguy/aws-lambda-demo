...
// ~AIB
{
    ...
    throws ProcessingException
    {
	    ...
	    StringBuilder fromClause = new StringBuilder( "from com.harbormaster.bo.League as league where " );
...
// AIB : #getHibernateFindFromClause()
			fromClause.append( "league.leagueId = " + pk.getLeagueId().toString() );
// ~AIB
		...
		finally
		{
			...
			{		
				LOGGER.info( "LeagueDAO.findLeague - Hibernate failed to close the Session - " + exc );
			}
		}		    
	    ...
	    throws ProcessingException
	    {
			...
			finally
			{
				...
				{		
					LOGGER.info( "LeagueDAO.findAllLeague - Hibernate failed to close the Session - " + exc );
				}		
			}
	    ...
	    throws ProcessingException
	    {
			...
			catch( Throwable exc )
			{
				...
				{
					LOGGER.info( "LeagueDAO.createLeague - Hibernate failed to rollback - " + exc1 );
				}
				exc.printStackTrace();			
				throw new ProcessingException( "LeagueDAO.createLeague failed - " + exc );
			}		
			finally
			{
				try
				...
				{		
					LOGGER.info( "LeagueDAO.createLeague - Hibernate failed to close the Session - " + exc );
				}
			}		
    ...
    throws ProcessingException
    {
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "LeagueDAO.saveLeague - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
			throw new ProcessingException( "LeagueDAO.saveLeague failed - " + exc );		
		}		
		finally
		{
			try
			...
			{		
				LOGGER.info( "LeagueDAO.saveLeague - Hibernate failed to close the Session - " + exc );
			}
		}		    
        return( businessObject );
    }
    ...
    * @exception    ProcessingException
    */
    public void deleteLeague( LeaguePrimaryKey pk ) 
    throws ProcessingException 
    {
	    Transaction tx 	= null;
	    Session session = null;
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "LeagueDAO.deleteLeague - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
		...
		finally
		{
			...
			{		
				LOGGER.info( "LeagueDAO.deleteLeague - Hibernate failed to close the Session - " + exc );
			}
		}
    }
