...
// ~AIB
{
    ...
    throws ProcessingException
    {
	    ...
	    StringBuilder fromClause = new StringBuilder( "from com.harbormaster.bo.Matchup as matchup where " );
...
// AIB : #getHibernateFindFromClause()
			fromClause.append( "matchup.matchupId = " + pk.getMatchupId().toString() );
// ~AIB
		...
		finally
		{
			...
			{		
				LOGGER.info( "MatchupDAO.findMatchup - Hibernate failed to close the Session - " + exc );
			}
		}		    
	    ...
	    throws ProcessingException
	    {
			...
			Query query 							= null;		
			StringBuilder buf 						= new StringBuilder( "from com.harbormaster.bo.Matchup" );
			...
			finally
			{
				...
				{		
					LOGGER.info( "MatchupDAO.findAllMatchup - Hibernate failed to close the Session - " + exc );
				}		
			}
	    ...
	    throws ProcessingException
	    {
			...
			catch( Throwable exc )
			{
				...
				{
					LOGGER.info( "MatchupDAO.createMatchup - Hibernate failed to rollback - " + exc1 );
				}
				exc.printStackTrace();			
				throw new ProcessingException( "MatchupDAO.createMatchup failed - " + exc );
			}		
			finally
			{
				try
				...
				{		
					LOGGER.info( "MatchupDAO.createMatchup - Hibernate failed to close the Session - " + exc );
				}
			}		
    ...
    throws ProcessingException
    {
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "MatchupDAO.saveMatchup - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
			throw new ProcessingException( "MatchupDAO.saveMatchup failed - " + exc );		
		}		
		finally
		{
			try
			...
			{		
				LOGGER.info( "MatchupDAO.saveMatchup - Hibernate failed to close the Session - " + exc );
			}
		}		    
        return( businessObject );
    }
    ...
    * @exception    ProcessingException
    */
    public void deleteMatchup( MatchupPrimaryKey pk ) 
    throws ProcessingException 
    {
	    Transaction tx 	= null;
	    Session session = null;
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "MatchupDAO.deleteMatchup - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
		...
		finally
		{
			...
			{		
				LOGGER.info( "MatchupDAO.deleteMatchup - Hibernate failed to close the Session - " + exc );
			}
		}
    }
