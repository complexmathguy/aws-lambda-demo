...
// ~AIB
{
    ...
    throws ProcessingException
    {
	    ...
	    StringBuilder fromClause = new StringBuilder( "from com.harbormaster.bo.Game as game where " );
...
// ~AIB
		...
		finally
		{
			...
			{		
				LOGGER.info( "GameDAO.findGame - Hibernate failed to close the Session - " + exc );
			}
		}		    
	    ...
	    throws ProcessingException
	    {
			...
			finally
			{
				...
				{		
					LOGGER.info( "GameDAO.findAllGame - Hibernate failed to close the Session - " + exc );
				}		
			}
	    ...
	    throws ProcessingException
	    {
			...
			catch( Throwable exc )
			{
				...
				{
					LOGGER.info( "GameDAO.createGame - Hibernate failed to rollback - " + exc1 );
				}
				exc.printStackTrace();			
			...
			finally
			{
				...
				{		
					LOGGER.info( "GameDAO.createGame - Hibernate failed to close the Session - " + exc );
				}
			}		
    ...
    throws ProcessingException
    {
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "GameDAO.saveGame - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
		...
		finally
		{
			...
			{		
				LOGGER.info( "GameDAO.saveGame - Hibernate failed to close the Session - " + exc );
			}
		}		    
        return( businessObject );
    }
    ...
    * @exception    ProcessingException
    */
    public void deleteGame( GamePrimaryKey pk ) 
    throws ProcessingException 
    {
	    Transaction tx 	= null;
	    Session session = null;
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "GameDAO.deleteGame - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
		...
		finally
		{
			...
			{		
				LOGGER.info( "GameDAO.deleteGame - Hibernate failed to close the Session - " + exc );
			}
		}
    }
