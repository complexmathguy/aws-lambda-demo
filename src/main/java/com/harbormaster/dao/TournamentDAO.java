...
// ~AIB
{
    ...
    throws ProcessingException
    {
	    ...
	    StringBuilder fromClause = new StringBuilder( "from com.harbormaster.bo.Tournament as tournament where " );
...
// AIB : #getHibernateFindFromClause()
			fromClause.append( "tournament.tournamentId = " + pk.getTournamentId().toString() );
// ~AIB
		...
		finally
		{
			...
			{		
				LOGGER.info( "TournamentDAO.findTournament - Hibernate failed to close the Session - " + exc );
			}
		}		    
	    ...
	    throws ProcessingException
	    {
			...
			Query query 							= null;		
			StringBuilder buf 						= new StringBuilder( "from com.harbormaster.bo.Tournament" );
			...
			finally
			{
				...
				{		
					LOGGER.info( "TournamentDAO.findAllTournament - Hibernate failed to close the Session - " + exc );
				}		
			}
	    ...
	    throws ProcessingException
	    {
			...
			catch( Throwable exc )
			{
				...
				{
					LOGGER.info( "TournamentDAO.createTournament - Hibernate failed to rollback - " + exc1 );
				}
				exc.printStackTrace();			
				throw new ProcessingException( "TournamentDAO.createTournament failed - " + exc );
			}		
			finally
			{
				try
				...
				{		
					LOGGER.info( "TournamentDAO.createTournament - Hibernate failed to close the Session - " + exc );
				}
			}		
    ...
    throws ProcessingException
    {
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "TournamentDAO.saveTournament - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
			throw new ProcessingException( "TournamentDAO.saveTournament failed - " + exc );		
		}		
		finally
		{
			try
			...
			{		
				LOGGER.info( "TournamentDAO.saveTournament - Hibernate failed to close the Session - " + exc );
			}
		}		    
        return( businessObject );
    }
    ...
    */
    ...
    throws ProcessingException 
    {
		...
		catch( Throwable exc )
		{
			...
			{
				LOGGER.info( "TournamentDAO.deleteTournament - Hibernate failed to rollback - " + exc1 );
			}
			exc.printStackTrace();			
		...
		finally
		{
			...
			{		
				LOGGER.info( "TournamentDAO.deleteTournament - Hibernate failed to close the Session - " + exc );
			}
		}
    }
